...
/* OPTIONS */
static int showprefix = 0; /* -prefix --show_prefix (false) Show installation path head */
static int version = 0; /* -version --show_version (false) Show full osc version number */
static int majorversion = 0; /* -majorversion --show_majorversion (false) Show short osc version number */
static int fileio = 0; /* -fileio --file_io (false) Use stdin/stdout to read/write strings */
static int inlining = 1; /* -x --noinline (true) Supress auto inlining */
...
static int gprof; /* -pg --profiling (false) Compile/link intermediate C code with -pg for profiling. */
...
static int sliceThrottle; /* -<no->slice-throttle (false) Control slice throttle feature that runs short parallel loops serially (on by default) */
...
static int assumedIterations = 1000; /* -iter=count --assumed-iterations=count () Assume this many iterations for loops to drive partition */
...
static option_t options[] = {
   {"",0,overviewOption,"SISAL compiler",
    "sisalc compiles Sisal 1.2 programs generating IF1, IF2, C, or executable programs\n",
    0,0,0,0},
   {"--help","--usage",optionHelp,"Display information on options and arguments",
    "Display information on options and arguments",
    0,0,0,0},
   {"--html",0,optionHTML,"Display information on options and arguments as HTML",
    "Display information on options and arguments as HTML",
    0,0,0,0},
   {"--man",0,optionMAN,"Display information on options and arguments as man nroff",
    "Display information on options and arguments as man nroff",
    0,0,0,0},

   /* OPTIONS */
    /* Autogenerated using generateOptions.py on Sun Jun 27 05:03:52 2010 */
    {"-prefix","--show_prefix",defaultFalse,"Show installation path head",
     "Show installation path head",
     &showprefix,0,0,0},
    {"-version","--show_version",defaultFalse,"Show full osc version number",
     "Show full osc version number",
     &version,0,0,0},
    {"-majorversion","--show_majorversion",defaultFalse,"Show short osc version number",
     "Show short osc version number",
     &majorversion,0,0,0},
    {"-fileio","--file_io",defaultFalse,"Use stdin/stdout to read/write strings",
     "Use stdin/stdout to read/write strings",
     &fileio,0,0,0},
    {"-x","--noinline",defaultTrue,"Supress auto inlining",
     "Supress auto inlining",
     &inlining,0,0,0},
    ...
    {"-n","--just-print",defaultFalse,"Don't actually run any commands, just print them.",
     ...
     &debug,0,0,0},
    {"-pg","--profiling",defaultFalse,"Compile/link intermediate C code with -pg for profiling.",
     "Compile/link intermediate C code with -pg for profiling.",
     &gprof,0,0,0},
    ...
    {"-module","--module",defaultFalse,"Suppress warning messages",
     "Suppress warning messages",
    ...
    {"-<no->bounds",0,defaultTrue,"Control bounds checking (on by default)",
     ...
     &bounds,0,0,0},
    {"-<no->slice-throttle",0,defaultFalse,"Control slice throttle feature that runs short parallel loops serially (on by default)",
    ...
    {"-p","--processors",defaultInitialized,"Partition for this many cpus 1",
     ...
     &processors,0,0,0},
    {"-iter","--assumed-iterations",defaultInitialized,"Assume this many iterations for loops to drive partition 1000",
     "Assume this many iterations for loops to drive partition 1000",
    ...
    {"LDFLAGS",0,fetchStringEqual,"Add to link/loader",
     "Add to link/loader",
     0,0,&LDFLAGS,0},
    {"-o",0,fetchStringNext,"Executable (default s.out) or object output file name",
     "Executable (default s.out) or object output file name",
     0,0,&target,0},
...
int main(int argc, char** argv) {
   ...
   /* ----------------------------------------------- */
   if ( gprof ) {
     enqueue(&loadOptions,"-pg");
   }

   if(stopObject && target) {
      if(queueSize(cFiles) + queueSize(f77Files) > 1)
         compilerError("cannot use -o with -c and multiple source files including C/F77 sources");
   }
   ...
   while(cFiles) {
      ...
      compile(file,result,includeDirectory,gprof,CC,CFLAGS);
   ...
   while(f77Files) {
      ...
      if(stopObject && target)
         ...
         result = generateFilename(file,".o",stopObject);
      compile(file,result,(SISAL_F77_DASHI_OK)?(includeDirectory):(0),profiling,F77,SISAL_FFLAGS);
      enqueue(&objectFiles,result);
   }
   ...
   if ( objectFiles ) {
      linker(LD,
             (target ? target : DEFAULT_OUTPUT_NAME),
             libDirectory,
             &objectFiles,
             &loadOptions,
             LDFLAGS,
      ...
      while(loadOptions) {
         file = dequeue(&loadOptions);
         fprintf(stderr," %s",file);
      }
      fputc('\n',stderr);
      compilerError("produces no object files.  use --help for options");
   }
