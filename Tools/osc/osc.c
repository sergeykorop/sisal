...
staticforward void RemoveFile();

static char BIN_PATH[2048];
static char DATA_PATH[2048];
static char INC_PATH[2048];
static char LIB_PATH[2048];
static char DASHL_LIB_PATH[2048];
...
#define StrCat3( r, s1, s2, s3 ) strcat( strcat( strcpy( (r), (s1) ), (s2) ), s3 )
...
static int SubmitCommands()
{
  ...
  case FTYPE_LD:
	...
	av[avcnt++] = srt0;
      ...
      av[avcnt++] = DASHL_LIB_PATH;
      av[avcnt++] = "-lsisal";

      /* ----------------------------------------------- */
      /* Add configure detected libraries                */
      /* ----------------------------------------------- */
      AddAV(SISAL_LIBS);
      AddAV(SISAL_LDFLAGS);
    }
  ...
  case FTYPE_F:
    ...
    av[avcnt++] = "-lsisal";
      ...
      /* ----------------------------------------------- */
      av[avcnt++] = SISAL_LIBS;
      av[avcnt++] = SISAL_LDFLAGS;
      av[avcnt++] = SISAL_FLIBS;
...
static char VERSION[] = SISAL_VERSION;
#ifndef VPREFIX
# define VPREFIX ""
#endif
...
char **argv;
{
  ...
  if (verbose) 
      printf("OSC %s %s\n", SISAL_VERSION, VPREFIX);

  /* ----------------------------------------------- */
  /* Figure out the major version                    */
  /* ----------------------------------------------- */
  majorVersion = (char*)malloc(strlen(VERSION)+1);
  for(p=VERSION; *p ; ++p) {
     if ( *p == '.' ) break;
  }
  strncpy(majorVersion,VERSION,p-VERSION);
  majorVersion[strlen(VERSION)] = 0;
     
  
  /* ------------------------------------------------------------ */
  StrCat3(BIN_PATH,  SISAL_PATH_PREFIX,  "/bin/sisal", majorVersion );
  StrCat3(INC_PATH,  SISAL_PATH_PREFIX,  "/include/sisal", majorVersion );
  StrCat3(LIB_PATH,  SISAL_PATH_PREFIX,  "/lib/sisal", majorVersion );
  StrCat(DASHL_LIB_PATH,  "-L", LIB_PATH);
  StrCat3(DATA_PATH, SISAL_PATH_PREFIX,  "/share/sisal", majorVersion );
  ...
  StrCat( sisalpt,   DATA_PATH,  "" );	/* etablebin, ptablebin */
  ...
  StrCat( spp2,      BIN_PATH,  "/spp2"      );
    ...
    StrCat( srt0,      LIB_PATH,  "/srt0.o"    );

    if ( costs == NULL ) {
        costs = buf1;
        StrCat( costs,   DATA_PATH,  "/s.costs"   );
    }
...
/* $Log$
/* Revision 1.1.1.1  2000/12/31 17:54:49  patmiller
